...
describe('(Component) Settings', () => {
  ...
  beforeEach(() => {
    ...
    _wrapper = shallow(<Settings {..._props} />)
  })
  ...
  it('Should render with an <h2> that includes Sample Settings text.', () => {
    expect(_wrapper.find('h2').text()).to.match(/Settings:/)
  })

  it('Should render props.settings at the end of the sample settings <h2>.', () => {
    expect(_wrapper.find('h2').text()).to.match(/5$/)
    _wrapper.setProps({ settings: 8 })
