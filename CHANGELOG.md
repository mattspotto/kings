...
* Added sample tests for settings redux module
...
* Refactored sample settings constants and actions to their appropriate locations (thanks [kyleect](https://github.com/kyleect))
...
* Updates settings example to demonstrate `mapDispatchToProps`
...
#### Improvements
* Server no longer modifies initial settings state by default.
...
* Example store and view are now more intuitive (simple settings display).
...
* Bundle compilation now fails if an eslint error is ensettingsed when running a production build.
